...
for source_ip in source_addresses:                      # Loop through the source and destination IP addresses and ports
    print ("Source IP: " + source_ip + '\n')
    for dest_ip in destination_addresses:
        print ("Destination IP: " + dest_ip)
        print ('---------------------------------')
        for dest_port in destination_ports:
            port = destination_ports[dest_port]
            asdm.set_ptrace_data(source_ip, dest_ip, destination_ports[dest_port])
            asdm.asdm_call()

            output = "Dest Port: " + dest_port
